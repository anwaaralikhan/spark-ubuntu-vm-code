package com.dc.spark.app.learn;
...
public class TransformationService implements Serializable {
	...
	public void transform() {
		...
		/**
		 * setMaster should point to master-url for distributed clusters local = jobs
		 * will run in single jvm, mostly used for debugging purposes.
		 */
		...
		mappedRDD.foreach(x -> LOGGER.info(" After Map transformation : " + x));
		...
		mapPartitions.foreach(x -> LOGGER.info(" After Map transformation without index : " + x));
		...
		mapPartitionsWithIndex.foreach(x -> LOGGER.info(" After Map transformation with index : " + x));

		// filter RDD
		JavaRDD<Integer> filter = intRDD.filter(x -> (x % 2 == 0));
		mapPartitionsWithIndex.foreach(x -> LOGGER.info(" After filter (x % 2==0) : " + x));
		...
		flatMap.foreach(x -> LOGGER.info(" Split String : " + x));
		// map to pair
		...
		pairRDD.foreach(x -> LOGGER.info(" Even Odd Flat Mapping : " + x));
		// flat map to pair
